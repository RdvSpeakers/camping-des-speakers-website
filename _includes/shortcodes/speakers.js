 ...
 */
  ...
  */
 eleventyConfig.addShortcode('speakers', function (data, arr) {

   var l10n = data.site[data.locale]
   return `<section class="grid gap gup">
   ${arr
     ...
     `<article class="card team" style="${(item.data.highlight !== undefined && item.data.highlight === true)
       ...
       <div class="card_content">
         <div class="card_header">
           <h2 class="no-margin">
             <a href="${this.url(`/${data.site[data.locale].speakers.url }/${item.data.key}/`)}">
               ${item.data.name}
             </a>
           </h2>
           ${
